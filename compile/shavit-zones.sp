...
#define EF_NODRAW 32
#define SOLID_BBOX 2
#define TF_ROCKETJUMPER 237
#define TF_STICKYJUMPER 265
...
public void OnPluginStart()
{
	...
	// menu
	RegAdminCmd("sm_addzone", Command_Zones, ADMFLAG_KICK, "Opens the mapzones menu.");
	RegAdminCmd("sm_zones", Command_Zones, ADMFLAG_KICK, "Opens the mapzones menu.");
	...
	RegAdminCmd("sm_modifyzone", Command_ZoneEdit, ADMFLAG_KICK, "Modify an existing zone. Alias of sm_zoneedit.");
	RegAdminCmd("sm_hookzone", Command_HookZone, ADMFLAG_KICK, "Hook an existing trigger, teleporter, or button.");

	RegAdminCmd("sm_tptozone", Command_TpToZone, ADMFLAG_KICK, "Teleport to a zone");
...
public int Native_InsideZoneStage(Handle handler, int numParams)
{
	...
	for (int i = 0; i < gI_MapZones; i++)
	{
		if(gB_InsideZoneID[client][i] && (gA_ZoneCache[i].iType == Zone_Stage || gA_ZoneCache[i].iType == Zone_Start) && gA_ZoneCache[i].iTrack == iTrack)
		{
...
public any Native_IsStageValid(Handle plugin, int numParams)
{
	int track = GetNativeCell(1);

	if (Shavit_ZoneExists(Zone_Start, track) && Shavit_ZoneExists(Zone_End, track))
		return true;
	else
		return false;
}
...
public int Native_TeleportToStartZone(Handle plugin, int numParams)
{
	TeleportToStartZone(GetNativeCell(1), GetNativeCell(2), GetNativeCell(3));
	return 0;
}
...
bool JumpToZoneType(KeyValues kv, int type, int track)
{
	...
	if (track == Track_Main)
	{
		FormatEx(key[0], sizeof(key[]), "Stage %d %s", track, config_keys[type][0]);
		if (track == Track_Main)
			FormatEx(key[1], sizeof(key[]), "Stage %s", config_keys[type][0]);

		if (config_keys[type][0][0])
		{
			FormatEx(key[2], sizeof(key[]), "Stage %d %s", track, config_keys[type][1]);
			if (track == Track_Main)
				FormatEx(key[3], sizeof(key[]), "Stage %s", config_keys[type][1]);
		}
	}
...
public Action Command_SetStart(int client, int args)
{
	...
	{
		Shavit_PrintToChat(client, "%T", "SetStartCommandAlive", client, gS_ChatStrings.sVariable, gS_ChatStrings.sText);
	...
	if(stage == 1)
	{
		FormatEx(sTrack, sizeof(sTrack), "%s", TRACK_NAME);
	}
	else
	{
...
public Action Command_DeleteSetStart(int client, int args)
{
	...
	for (int i = 0; i < TRACKS_SIZE; i++)
	{
		...
		{
			char info[8], sTrack[64];
			IntToString(i, info, sizeof(info));
			menu.AddItem(info, currentTrackInfo[i].trackName);
		}
	}
...
public int MenuHandler_DeleteSetStart(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		...
		Shavit_PrintToChat(param1, "%T", "DeleteSetStart", param1, gS_ChatStrings.sVariable, currentTrackInfo[track].trackName, gS_ChatStrings.sText, gS_ChatStrings.sWarning, gS_ChatStrings.sText);
...
Action DisplayCustomSpawnMenu(int client)
{
	...
	for(int i = 0; i < TRACKS_SIZE; i++)
	{
		...
		menu.AddItem(sInfo, currentTrackInfo[i].trackName);
	}
...
public int MenuHandler_AddCustomSpawn(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		...
		{
			Shavit_PrintToChat(param1, "%T", "ZoneCustomSpawnExists", param1, gS_ChatStrings.sVariable, currentTrackInfo[iTrack].trackName, gS_ChatStrings.sText);
...
Action DisplayCustomSpawnDeleteMenu(int client)
{
	...
	for(int i = 0; i < TRACKS_SIZE; i++)
	{
		...
		menu.AddItem(sInfo, currentTrackInfo[i].trackName, (EmptyVector(gF_CustomSpawn[i]))? ITEMDRAW_DISABLED:ITEMDRAW_DEFAULT);
	}
...
public int MenuHandler_DeleteCustomSpawn(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		...
		if(EmptyVector(gF_CustomSpawn[iTrack]))
		{
			Shavit_PrintToChat(param1, "%T", "ZoneCustomSpawnMissing", param1, gS_ChatStrings.sVariable, currentTrackInfo[iTrack].trackName, gS_ChatStrings.sText);
			return 0;
		}
...
public Action Command_Stages(int client, int args)
{
	...
	int iLastStage = Shavit_GetClientLastStage(client);
	...
	for(int i = 0; i < gI_MapZones; i++)
	{
		...
		{
			FormatEx(sDisplay, 64, "#%d - %T (%s)", (i + 1), "ZoneSetStage", client, gA_ZoneCache[i].iData, currentTrackInfo[gA_ZoneCache[i].iTrack].trackName);
		...
		{
			FormatEx(sDisplay, 64, "#%d - %T (%s)", (i + 1), "ZoneSetStage", client, 1, currentTrackInfo[gA_ZoneCache[i].iTrack].trackName);
...
public Action Command_Zones(int client, int args)
{
	...
	for(int i = 0; i <= Track_Bonus_Last; i++)
	{
		char sInfo[8];
		IntToString(i, sInfo, 8);
		menu.AddItem(sInfo, currentTrackInfo[i].trackName);
...
public int MenuHandler_SelectZoneTrack(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		...
		submenu.SetTitle("%T\n ", "ZoneMenuTitle", param1, currentTrackInfo[gA_EditCache[param1].iTrack].trackName);
...
Action OpenTpToZoneMenu(int client, int pagepos=0)
{
	...
	for (int i = 0; i < gI_MapZones; i++)
	{
		...
		{
			FormatEx(sDisplay, sizeof(sDisplay), "#%d - %s %d (%s)%s", (i + 1), sZoneName, gA_ZoneCache[i].iData, currentTrackInfo[gA_ZoneCache[i].iTrack].trackName, sTarget);
		}
		else if (gA_ZoneCache[i].iType == Zone_Gravity || gA_ZoneCache[i].iType == Zone_Speedmod)
		{
			FormatEx(sDisplay, 64, "#%d - %s %.2f (%s)", (i + 1), sZoneName, gA_ZoneCache[i].iData, currentTrackInfo[gA_ZoneCache[i].iTrack].trackName);
		}
		else
		{
			FormatEx(sDisplay, sizeof(sDisplay), "#%d - %s (%s)%s", (i + 1), sZoneName, currentTrackInfo[gA_ZoneCache[i].iTrack].trackName, sTarget);
		}
...
Action OpenEditMenu(int client, int pos = 0)
{
	...
	for(int i = 0; i < gI_MapZones; i++)
	{
		...
		if (gA_ZoneCache[i].iType == Zone_CustomSpeedLimit || gA_ZoneCache[i].iType == Zone_Stage || gA_ZoneCache[i].iType == Zone_Airaccelerate || gA_ZoneCache[i].iType == Zone_Checkpoint)
		{
			FormatEx(sDisplay, sizeof(sDisplay), "#%d - %s %d (%s)%s", (i + 1), sZoneName, gA_ZoneCache[i].iData, currentTrackInfo[gA_ZoneCache[i].iTrack].trackName, sTarget);
		}
		else if (gA_ZoneCache[i].iType == Zone_Gravity || gA_ZoneCache[i].iType == Zone_Speedmod)
	...
	for (int i = 0; i < TRACKS_SIZE; i++)
	{
...
#endif
		{
			if (j != Zone_CustomSpawn)// && gA_ZoneSettings[j][i].bVisible)
			{
				...
				FormatEx(display, sizeof(display), "%s - %s", currentTrackInfo[i].trackName, zoneName);
				menu.AddItem(info, display);
			}
		}
	}
...
void OpenSubCustomZoneMenu(int client, int track, int zoneType)
{
	...
	menu.SetTitle("%T", "CustomZone_SubMenuTitle", client, currentTrackInfo[track].trackName, zoneName);
	...
	if(action == MenuAction_Select)
	{
		...
		switch(id)
		{
			...
			default:
			{
				...
				hDatapack.WriteCell(gA_ZoneCache[id].iTrack);
				hDatapack.WriteCell(gA_ZoneCache[id].iData);
...
public void CallOnZoneDeleted(int type, int track, int data, bool all)
{
	if(!all)
	{
		char sTrack[64];
		...
		if (type == Zone_Start || type == Zone_End)
		{
			for(int i = 0; i < MaxClients; i++)
			{
				if(IsValidClient(i) && Shavit_GetClientTrack(i) == track)
				{
					...
					Shavit_PrintToChat(i, "%T", "ZoneDeletedStopTimer", i, gS_ChatStrings.sVariable, currentTrackInfo[track].trackName, gS_ChatStrings.sText, sZoneName);
...
public void SQL_DeleteZone_Callback(Database db, DBResultSet results, const char[] error, DataPack data)
{
	...
	CallOnZoneDeleted(type, track, iData, false);
...
public int MenuHandler_SelectZoneType(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		...
		Shavit_PrintToChat(param1, "%T", "ZoneCreateType", param1, gS_ChatStrings.sVariable, sZoneName, gS_ChatStrings.sText, gS_ChatStrings.sVariable2, currentTrackInfo[gA_EditCache[param1].iTrack].trackName);
...
void CreateEditMenu(int client, bool autostage=false)
{
	...
	if (hookmenu)
	{
		menu.SetTitle("%T\n%T\n%T\n%T\n ",
			...
			"ZoneEditTrack", client, currentTrackInfo[gA_EditCache[client].iTrack].trackName,
...
void InsertZone(int client)
{
	...
	if (c.iDatabaseID == -1) // insert
	{
		Shavit_LogMessage(
			...
			client, currentTrackInfo[c.iTrack].trackName, sZoneName, gS_Map,
		...
		FormatEx(sQuery, sizeof(sQuery),
			"INSERT INTO %smapzones (map, type, track_name, \
			...
			%d, %d, '%s');",
			gS_MySQLPrefix,
			gS_Map, c.iType, currentTrackInfo[c.iTrack].trackName,
			EXPAND_VECTOR(c.fCorner1),
			EXPAND_VECTOR(c.fCorner2),
			EXPAND_VECTOR(c.fDestination),
	...
	else // update
	{
		Shavit_LogMessage(
			...
			p1(%f, %f, %f), p2(%f, %f, %f), dest(%f, %f, %f), \
			flags=%d, data=%d, speedlimit=%d, form=%d, target='%s'",
			client, currentTrackInfo[c.iTrack].trackName, sZoneName, c.iDatabaseID, gS_Map,
			EXPAND_VECTOR(c.fCorner1),
			EXPAND_VECTOR(c.fCorner2),
			EXPAND_VECTOR(c.fDestination),
			c.iFlags, c.iData, view_as<int>(c.bUseSpeedLimit),
...
public void Shavit_OnRestart(int client, int track, bool tostartzone)
{
	...
	if(gCV_TeleportToStart.BoolValue)
	{
		...
		else if (bCustomStart || iIndex != -1)
		{
			...
			if (!gB_HasSetStart[client][track] || gB_StartAnglesOnly[client][track][stage])
				ResetClientTargetNameAndClassName(client, track);
...
{
	bool bReplay = (gB_ReplayPlayback && Shavit_IsReplayEntity(other));
	
	if ((other < 1 || other > MaxClients || IsFakeClient(other)) && !bReplay)
	{
		return;
	}
	...
	switch (type)
	{
		case Zone_Secret:
		{
			if(status != Timer_Stopped && !bReplay)
			{
				...
				Shavit_PrintToChat(other, "%T", "ZoneSecretEnter", other, gS_ChatStrings.sText, gS_ChatStrings.sVariable, gS_ChatStrings.sText, gS_ChatStrings.sWarning);
			}
		}
		...
		case Zone_Cheater:
		{
			...
			{
				Shavit_StopTimer(other);
				Shavit_PrintToChat(other, "%T", "ZoneCheaterEnter", other, gS_ChatStrings.sWarning);
			}
		}
		...
		case Zone_Stop:
		{
			...
			{
				Shavit_StopTimer(other);
				Shavit_PrintToChat(other, "%T", "ZoneStopEnter", other, gS_ChatStrings.sWarning);
			}
		}
		...
		case Zone_Checkpoint:
		{
			...
			int iLastCP = Shavit_GetClientLastStage(other);
...
public void TouchPost(int entity, int other)
{
	...
	switch (type)
	{
		...
		case Zone_Secret:
		{
			...
			{
				Shavit_StopTimer(other);
				Shavit_PrintToChat(other, "%T", "ZoneSecretEnter", other, gS_ChatStrings.sVariable2, gS_ChatStrings.sText, gS_ChatStrings.sVariable, gS_ChatStrings.sText, gS_ChatStrings.sWarning);
			}
		}
		case Zone_Teleport:
		{
